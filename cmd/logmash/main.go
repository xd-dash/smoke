...
// Run is Logmash's in-process Smoke command entry point. Stdout implies an
// interactive foreground lifetime. A runtime with stdout disabled is launched
// in the background unless --foreground explicitly keeps it attached.
func Run(args []string) error {
	...
	if err != nil { return fmt.Errorf("into: %w", err) }
	...
	background := !cfg.Stdout && !cfg.Foreground
	if background && os.Getenv("LOGMASH_BACKGROUND") != "1" {
		pid, err := detach(args)
		if err != nil { return fmt.Errorf("background: %w", err) }
		fmt.Fprintf(os.Stderr, "logmash: started background pid=%d; use `smoke logmash list` or `smoke logmash stop <session-id>`\n", pid)
		return nil
	}

	dispatcher, err := callback.Parse(callbackValues)
	if err != nil { return fmt.Errorf("callbacks: %w", err) }
	...
	if err != nil { return fmt.Errorf("sources: %w", err) }
	...
	if os.Getenv("LOGMASH_BACKGROUND") == "1" {
		...
		handle, record, err = session.Begin(session.Record{
			Profile: strings.Join(profiles, ","), Target: fmt.Sprintf("sources=%d", len(subscriptions)),
			Channels: qualifiedChannels, Patterns: qualifiedPatterns, Callbacks: callbackValues,
...
func resolveSources(ctx context.Context, cfg cliArgs) ([]sourceSubscription, error) {
	...
	for _, source := range sources {
		...
		if err != nil { return nil, fmt.Errorf("resolve %s (%s): %w", source, profile, err) }
		authProvider := cfg.AuthProvider
		if authProvider == "" { authProvider = target.AuthProvider }
		if authProvider == "" { authProvider = "auto-env" }
		authProfile := target.AuthProfile
		if authProfile == "" { authProfile = strings.ReplaceAll(source, ":", "-") }
		credentials, err := authRegistry.Resolve(ctx, authProvider, authProfile)
		if err != nil { return nil, fmt.Errorf("%s auth provider %s: %w", source, authProvider, err) }
...
func runSources(parent context.Context, subscriptions []sourceSubscription, dispatcher *callback.Dispatcher) error {
	...
	for _, source := range subscriptions {
		...
		go func() {
			...
			if err != nil && ctx.Err() == nil { errCh <- fmt.Errorf("source %s: %w", source.Source, err); cancel() }
		}()
	}
...
func sessionSelectors(subscriptions []sourceSubscription) (channels, patterns, profiles []string) {
	for _, source := range subscriptions {
		...
		for _, channel := range source.Channels { channels = append(channels, source.Source+":"+channel) }
		for _, pattern := range source.Patterns { patterns = append(patterns, source.Source+":"+pattern) }
	}
...
func sessionAuthProvider(subscriptions []sourceSubscription) string {
	...
	for _, source := range subscriptions { providers[source.AuthProvider] = struct{}{} }
...
func displayTarget(target redisprovider.Target) string {
	if target.Network == "unix" { return "target=" + target.Socket + " network=unix" }
...
func parseArgs(args []string) (cliArgs, error) {
	...
	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--pattern", "-p":
			i++; if i >= len(args) { return cfg, fmt.Errorf("%s requires COUNTRY:REGION:PATTERN", args[i-1]) }
			selector, err := parseSourceSelector(args[i], true); if err != nil { return cfg, err }; cfg.Sources = append(cfg.Sources, selector)
		case "--into":
			if i+3 >= len(args) { return cfg, fmt.Errorf("--into requires PROVIDER PROFILE TARGET") }
			cfg.Into = append(cfg.Into, intoSpec{Provider: args[i+1], Profile: args[i+2], Target: args[i+3]}); i += 3
		case "--callback", "-c":
			i++; if i >= len(args) { return cfg, fmt.Errorf("%s requires a value", args[i-1]) }; cfg.Callbacks = append(cfg.Callbacks, args[i])
		case "--callback-policy":
			i++; if i >= len(args) { return cfg, fmt.Errorf("--callback-policy requires a value") }; cfg.Policy = callback.FailurePolicy(args[i])
		case "--auth-provider":
			i++; if i >= len(args) { return cfg, fmt.Errorf("--auth-provider requires a value") }; cfg.AuthProvider = args[i]
		case "--foreground": cfg.Foreground = true
	...
	if len(cfg.Sources) == 0 { return cfg, fmt.Errorf("usage: logmash COUNTRY:REGION:CHANNEL [COUNTRY:REGION:CHANNEL ...] [--pattern COUNTRY:REGION:GLOB] [--foreground] [--no-stdout] [--into PROVIDER PROFILE TARGET]") }
	return cfg, nil
}
...
func parseSourceSelector(value string, pattern bool) (sourceSelector, error) {
	...
	country, region, item := strings.ToLower(strings.TrimSpace(parts[0])), strings.ToLower(strings.TrimSpace(parts[1])), strings.TrimSpace(parts[2])
...
func invalidSourceSelector(value string, pattern bool) (sourceSelector, error) {
	kind := "CHANNEL"; if pattern { kind = "PATTERN" }
