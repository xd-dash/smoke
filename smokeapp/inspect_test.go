...
func TestInspectCommandRuns(t *testing.T) {
	...
	if err := Run([]string{"inspect", "--json"}); err != nil {
		t.Fatal(err)
	}
}
...
func TestRuntimeInspectionDocument(t *testing.T) {
	...
	if doc.Kind != "smoke.runtime" {
		t.Fatalf("kind = %q", doc.Kind)
	}
	if doc.Composition.Digest == "" {
	...
	if doc.Runtime.Workspace != "/tmp/smoke/workspaces/abc123/go.work" {
...
func TestEnvInspectCreatesRuntimeSnapshot(t *testing.T) {
	...
	if err := Run([]string{"env", "inspect", "infra"}); err != nil {
		t.Fatal(err)
	}
	if err := Run([]string{"env", "inspect", "infra", "--json"}); err != nil {
		t.Fatal(err)
	}
	...
	if doc.Kind != "smoke.environment" {
		t.Fatalf("kind = %q", doc.Kind)
	}
	if doc.Environment.Name != "infra" {
		t.Fatalf("name = %q", doc.Environment.Name)
	}
	if doc.RuntimeSnapshot.Digest == "" {
		t.Fatal("missing workspace digest")
	}
	if doc.RuntimeSnapshot.Work == "" || doc.RuntimeSnapshot.Tools == "" {
		t.Fatal("missing workspace paths")
	}
}
...
func TestInspectionFlagParsing(t *testing.T) {
	...
	if err != nil || name != "infra" || !jsonOutput {
		t.Fatalf("env inspect args: name=%q output=%v err=%v", name, jsonOutput, err)
	}
	name, jsonOutput, err = parseEnvironmentInspectArgs([]string{"--json", "infra"})
	if err != nil || name != "infra" || !jsonOutput {
		t.Fatalf("env inspect prefix args: name=%q output=%v err=%v", name, jsonOutput, err)
	}
}
