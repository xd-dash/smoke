...
// Run executes the Logmash command registered into a composed Smoke binary.
func Run(args []string) error {
	if len(args) > 0 {
		...
		case "list", "ls":
			...
			if len(args) == 2 && args[1] == "--verbose" {
				...
				return fmt.Errorf("usage: logmash list [--verbose]")
			}
	...
	if unattended(cfg) && os.Getenv("LOGMASH_BACKGROUND") != "1" {
		pid, err := startBackground(args)
		if err != nil {
			return fmt.Errorf("start unattended runtime: %w", err)
		}
	...
	if os.Getenv("LOGMASH_BACKGROUND") == "1" {
		channels, patterns, profiles := sessionSelectors(subscriptions)
...
func resolveSources(ctx context.Context, cfg cliArgs) ([]sourceSubscription, error) {
	...
	for _, selector := range cfg.Sources {
		...
		if selector.Pattern {
			current.patterns[selector.Value] = struct{}{}
		} else {
			current.channels[selector.Value] = struct{}{}
		}
	}
	...
	for _, source := range sources {
		current := groups[source]
		profile := sourceProfile(current.country, current.region)
		...
		out = append(out, sourceSubscription{
			Source:       source,
			Profile:      profile,
			Channels:     sortedKeys(current.channels),
			Patterns:     sortedKeys(current.patterns),
			Target:       target,
...
func parseArgs(args []string) (cliArgs, error) {
	...
	if len(cfg.Sources) == 0 {
		return cfg, fmt.Errorf("usage: logmash COUNTRY:REGION:CHANNEL [COUNTRY:REGION:CHANNEL ...] [--pattern COUNTRY:REGION:GLOB] [--no-stdout] [--attached] [--into PROVIDER PROFILE TARGET]")
	}
	return cfg, nil
}
