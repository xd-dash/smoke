...
func runSources(parent context.Context, subscriptions []sourceSubscription, dispatcher *callback.Dispatcher) error {
	...
	select { case err := <-errCh: <-done; return err; case <-parent.Done(): cancel(); <-done; return nil; case <-done: select { case err := <-errCh: return err; default: return nil } }
}

func sessionSelectors(subscriptions []sourceSubscription) (channels, patterns, profiles []string) { for _, source := range subscriptions { profiles = append(profiles, source.Profile); for _, channel := range source.Channels { channels = append(channels, source.Source+":"+channel) }; for _, pattern := range source.Patterns { patterns = append(patterns, source.Source+":"+pattern) } }; return channels, patterns, profiles }
...
func listSessions(verbose bool) error {
	...
	for _, record := range records {
		...
		if verbose { fmt.Printf("  composition: %s\n", emptyLabel(record.CompositionDigest)); fmt.Printf("  environment: %s\n", emptyLabel(record.Environment)); fmt.Printf("  workspace digest: %s\n", emptyLabel(record.WorkspaceDigest)); fmt.Printf("  workspace: %s\n", emptyLabel(record.Workspace)); fmt.Printf("  lease: %s\n", emptyLabel(record.Lease)); fmt.Printf("  started: %s\n", record.StartedAt.Format("2006-01-02T15:04:05Z07:00")) }
	}
	return nil
}
