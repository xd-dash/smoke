...
//
// Each capability remains ordinary Go code or an ordinary Go tool. Smoke owns
// the named environment, immutable snapshot, and execution lifecycle.
// Credentials and organization-specific values are never persisted here.
package ghxd
...
// ToolSpecs is the default GitHub capability set. The worktree tool remains in
// the Smoke module because its reusable implementation is ghxd/worktree, while
// github-cdn and github-device-auth remain independently installable tools.
var ToolSpecs = []string{
	"github.com/dash-xd/github-cdn@go",
	"github.com/dash-xd/github-device-auth/cmd/github-device-auth@main",
	"github.com/xd-dash/smoke/cmd/github-worktree@main",
}
