...
func TestBeginCapturesRuntimeIdentity(t *testing.T) {
	...
	workspace := filepath.Join(t.TempDir(), "workspace-digest")
	...
	defer handle.Close()
	if record.CompositionDigest == "" {
		t.Fatal("missing composition digest")
	}
	if record.Environment != "infra" {
		t.Fatalf("environment=%q", record.Environment)
	}
	if record.WorkspaceDigest != "workspace-digest" {
		t.Fatalf("workspace digest=%q", record.WorkspaceDigest)
	}
	if record.Workspace != workspace {
		t.Fatalf("workspace=%q", record.Workspace)
	}
	if record.Lease == "" {
		t.Fatal("missing lease path")
	}
}
